...
import { SiteSetting, Stage } from '@lib/types';
import useLoginStatus from '@lib/hooks/use-login-status';
import styles from './stage-container.module.css';
import styleUtils from './utils.module.css';
import ScheduleSidebar from './schedule-sidebar';
import ConfEntry from './conf-entry';
import StageWarmpupView from './stage-warmup-view';
...
export default function StageContainer({ stage, allStages, siteSetting }: Props) {
  ...
  const warmupExercises = updatedStage.warmupExercises || [];
  const { loginStatus, mutate } = useLoginStatus();

  return (
    <div className={styles.container}>
      <div className={cn(styles.streamContainer, { [styles.expanded]: !updatedStage.live })}>
        { updatedStage && updatedStage.live ?
          loginStatus === 'loggedIn' ? (
            ...
            <ConfEntry onRegister={() => mutate()} siteSetting={siteSetting} />
          )
          :
          <StageWarmpupView warmupExercises={warmupExercises} />
        }
      </div>
      { updatedStage.live && <ScheduleSidebar allStages={updatedStages} /> }
    </div>
  );
}
